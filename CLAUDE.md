...
- `build-rootfs.sh` builds via Docker, then `docker export | gzip` produces a flat rootfs tarball suitable for `wsl --import`. **Note**: the "delta" layer extraction in `build_layer()` is a known approximation — it currently exports the full layered image rather than a true diff. Layers are installed by tar-extracting at `/` (see `scripts/install-layer.sh`), which uses `/opt/hermes/.layers/<name>.installed` as an idempotency marker.
...
- **Layer install isn't truly differential** — the layer tarballs duplicate core files. `install-layer.sh` extracts on top of an existing rootfs, which works because tar overwrites are idempotent, but layer sizes are larger than necessary.
